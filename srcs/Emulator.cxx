//
// Created by plouvel on 30/10/2025.
//

#include "Emulator.hxx"

#include <ranges>

#include "GbEmu.hxx"

const std::array<uint8_t, 256 + 52> Emulator::BOOT_ROM{
    };

Emulator::Emulator(utils::ThreadSafeQueue<Event>& eventQueue)
    : uiEventQueue(eventQueue),
      timer(bus),
      ppu{bus, framebuffer, [this] { onFramebufferReady(); }},
      cpu(bus, timer, ppu)
{
    bus.attach(cpu);
    bus.attach(timer);
    bus.attach(ppu);
    bus.attach(ram);

    for (const auto [i, byte] : std::views::enumerate(BOOT_ROM))
    {
        bus.write(BOOT_ROM_START_ADDRESS + i, byte);
    }
}

void Emulator::pushCommand(const Command& cmd)
{
    cmdQueue.push(cmd);
    cmdCv.notify_one();
}

void Emulator::onCpuMachineCycle()
{
    timer.tick();

    ppu.tick();
    ppu.tick();
    ppu.tick();
    ppu.tick();
}

void Emulator::onFramebufferReady() const
{
    pushEvent(Event::Type::FramebufferReady, Event::FramebufferReady{framebuffer});
}

void Emulator::pushEvent(const Event::Type type, Event::Payload payload) const
{
    uiEventQueue.push(Event{type, std::move(payload)});
}

void Emulator::operator()()
{
    pushEvent(Event::Type::Paused, Event::Paused{cpu.getView(), bus.getAddressSpace()});

    while (running)
    {
        while (true)
        {
            auto cmd{cmdQueue.try_pop()};
            if (!cmd.has_value())
            {
                break;
            }

            switch (cmd.value().type)
            {
                case Command::Type::Continue:
                    paused = false;
                    break;
                case Command::Type::SetBreakpoint:
                    breakpoints.insert(std::get<Command::Breakpoint>(cmd.value().payload).address);
                    break;
                case Command::Type::RemoveBreakpoint:
                    breakpoints.erase(std::get<Command::Breakpoint>(cmd.value().payload).address);
                    break;
                case Command::Type::WriteAddressSpace:
                {
                    const auto [address, byte] = std::get<Command::WriteAddressSpace>(cmd.value().payload);
                    bus.write(address, byte);
                }
                break;
                case Command::Type::Step:
                    paused         = false;
                    requestedPause = true;
                    break;
                case Command::Type::Exit:
                    return;
            }
        }

        if (paused)
        {
            std::unique_lock l{cmdMutex};
            cmdCv.wait(l);
            continue;
        }

        cpu.tick();

        if (auto [registers]{cpu.getView()}; breakpoints.contains(registers.PC))
        {
            requestedPause = true;
        }

        if (requestedPause)
        {
            paused         = true;
            requestedPause = false;

            pushEvent(Event::Type::Paused, Event::Paused{cpu.getView(), bus.getAddressSpace()});
        }
    }
}
